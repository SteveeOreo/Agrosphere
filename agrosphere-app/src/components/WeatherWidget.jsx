      ...
      <div className="bg-white rounded-lg shadow-sm border p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="font-semibold text-primary">{weatherData.current.temperature}°C</p>
              <p className="text-sm text-secondary">{weatherData.current.condition}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-xs text-muted">{location}</p>
            <p className="text-xs text-muted">Humidity: {weatherData.current.humidity}%</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="bg-white rounded-lg shadow-sm border p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div>
            <p className="text-3xl font-bold text-primary">{weatherData.current.temperature}°C</p>
            <p className="text-secondary">{weatherData.current.condition}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div className="text-center">
            ...
            <p className="font-semibold text-primary">{weatherData.current.humidity}%</p>
          </div>
          <div className="text-center">
            <p className="text-muted">Wind</p>
            <p className="font-semibold text-primary">{weatherData.current.windSpeed} km/h</p>
          </div>
          <div className="text-center">
            <p className="text-muted">UV Index</p>
            <p className="font-semibold text-primary">{weatherData.current.uvIndex}</p>
          </div>
        </div>
      </div>
      ...
      {/* 5-Day Forecast */}
      <div className="mb-6">
        <h4 className="font-semibold text-primary mb-3">5-Day Forecast</h4>
        <div className="space-y-2">
            ...
            <div key={index} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-secondary">{day.condition}</p>
                </div>
              </div>
              <div className="flex items-center space-x-4 text-sm">
                <div className="text-center">
                  <p className="text-muted">Rain</p>
                  <p className="font-medium text-info">{day.precipitation}%</p>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-primary">{day.high}°</p>
      ...
      {/* Farming Tips */}
      <div>
        <h4 className="font-semibold text-primary mb-3">Farming Recommendations</h4>
        <div className="space-y-2">
